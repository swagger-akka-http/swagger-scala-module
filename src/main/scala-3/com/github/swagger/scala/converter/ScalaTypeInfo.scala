...
/** The type information of a Scala 3 class that the JVM does not keep: the primitive types that the element types of its `Option`, `Map`
  * and collection fields have been erased to, and the subtypes of a sealed trait or class.
  *
  ...
  * library.
  *
  * Types with type parameters cannot be derived: the compiler asks for a `ScalaTypeInfo` of each type parameter, so the companion object
  * holds no instance that can be found by class. Register them instead - though a generic class has one runtime class for all of its type
  * arguments, so the element types that depend on one of its type parameters cannot be recorded either way.
  *
  * @since v2.16.0
  */
